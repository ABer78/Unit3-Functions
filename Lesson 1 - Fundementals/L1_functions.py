...
def calculate_engagement(likes, shares, comments):
    ...
    This function takes the number of likes, shares, and comments on a
    post and return the total engagemnet count.

    Args:
        likes (_type_): the number of likes
        shares (_type_): the number of shares
        comments (_type_): the number of comments

    Returning:
        int: the total engagement, calculated as the sum of likes, shares, and
        comments
...
def format_handle(username):
    return "@" + username  # could also be written as [return f"@{username}"]
